...
export default function PrayerTimeCard() {
  ...
  const calculatePrayerTimes = useCallback(() => {
    ...
    let prayerTimes = new AdhanPrayerTimes(coords, now, params);

    if (new Date(prayerTimes.isha) <= now) {
      const nextDay = new Date(now);
      nextDay.setDate(now.getDate() + 1);
      prayerTimes = new AdhanPrayerTimes(coords, nextDay, params);
    }
    ...
    Object.entries(todayTimings).forEach(([prayer, time]) => {
      ...
      if (msUntilPrayer > 0) {
        const timeoutId = setTimeout(() => {
          playAzaan(prayer);
          if (prayer === "Isha") {
            setTimeout(schedulePrayers, 60000);
          }
        }, msUntilPrayer);
  ...
  const startCountdown = useCallback(
    (givenTimings) => {
      ...
      countdownIntervalRef.current = setInterval(() => {
        ...
        const percent = Math.floor((passed / totalSeconds) * 100);
        setProgress(percent);
      }, 1000);
    },
    [timings, schedulePrayers, currentPrayer]
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full my-10 px-4 lg:px-0">
      ...
      <h2 className="text-3xl lg:text-4xl font-bold text-indigo-800 mb-2 text-center">Prayer Times</h2>
      ...
      <div className="bg-gradient-to-r from-indigo-700 to-purple-700 shadow-2xl rounded-lg p-6 lg:p-10 w-full max-w-md flex flex-col items-center border border-indigo-500 transition-all duration-500 hover:scale-105">
        {timings && (
              ...
              <thead>
                <tr className="border-b border-white/30">
                  {Object.keys(timings).map((prayer) => (
                    <th key={prayer} className="py-2 text-white">
                      {prayer}
                    </th>
                  ))}
                </tr>
        ...
        {currentPrayer && (
          <button
            onClick={stopAzaan}
            className="mt-6 px-6 py-3 bg-yellow-400 text-indigo-900 font-semibold rounded-full shadow-lg hover:bg-yellow-500 transition transform hover:scale-105 focus:outline-none focus:ring-4 focus:ring-yellow-300"
          >
